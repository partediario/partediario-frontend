...
interface RecorridaDrawerProps {
  isOpen?: boolean
  onClose?: () => void
  onSuccess?: () => void
...
export default function RecorridaDrawer({
  isOpen = false,
  onClose,
  onSuccess,
  actividadSeleccionada = null,
}: RecorridaDrawerProps) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true)
    try {
      const response = await fetch("/api/recorrida", {
        method: "POST",
      ...
      toast({
        title: "✅ Recorrida Guardada",
        description: `Se registraron ${detalles.length} potreros: ${potrerosOk} OK, ${potrerosConIncidentes} con incidentes`,
        duration: 4000,
      })

      // Disparar evento para recargar partes diarios
      console.log("Disparando evento reloadPartesDiarios después de guardar recorrida")
      window.dispatchEvent(new Event("reloadPartesDiarios"))

      handleClose()
      onSuccess?.()
    } catch (error) {
      console.error("Error saving recorrida:", error)
      toast({
        title: "❌ Error",
                  ...
                  {detalles.length === 0 ? (
                    ...
                    <div className="divide-y">
                      {detalles.map((detalle, index) => (
                        <div key={index} className="grid grid-cols-12 gap-4 p-3 text-sm hover:bg-gray-50 items-center">
