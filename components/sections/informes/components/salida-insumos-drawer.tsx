...
interface SalidaInsumosDrawerProps {
  isOpen: boolean
  onClose: () => void
  onSuccess?: () => void
}
...
export default function SalidaInsumosDrawer({ isOpen, onClose, onSuccess }: SalidaInsumosDrawerProps) {
  /* ------------------------- estados ------------------------- */
  ...
  const insumoSeleccionadoData = insumosExistentes.find((i) => i.insumo_id === insumoSeleccionado)
  const unidadMedidaSeleccionada = insumoSeleccionadoData?.unidad_medida || ""
  const cantidadDisponible = insumoSeleccionadoData?.cantidad_disponible || 0
  ...
  const cargarInsumosExistentes = async () => {
    ...
    try {
      const res = await fetch(`/api/insumos-existentes?establecimiento_id=${establecimientoSeleccionado}`)
      if (!res.ok) throw new Error("HTTP " + res.status)
      const data = await res.json()
      setInsumosExistentes(data.insumos ?? [])
    } catch (e) {
      console.error("Error cargando insumos existentes:", e)
      setInsumosExistentes([])
    } finally {
  ...
  const cargarTiposMovimiento = async () => {
    ...
    try {
      const res = await fetch(`/api/tipos-movimiento-insumos?direccion=SALIDA`)
      if (!res.ok) throw new Error("HTTP " + res.status)
      const data = await res.json()
  ...
  const validarFormulario = () => {
    ...
    // validar stock usando la cantidad disponible correcta
    if (cantidad > cantidadDisponible) {
      errores.push(`La cantidad solicitada (${cantidad}) supera el stock disponible (${cantidadDisponible})`)
    }
  ...
  const guardar = async () => {
    ...
    try {
      ...
      toast({
        title: "✅ Salida de Insumos Guardada",
  ...
  /* ------------------------- opciones combobox ------------------------- */
  // Solo mostrar el nombre del insumo, sin el stock
  const opcionesInsumos = insumosExistentes.map((i) => ({
    value: i.insumo_id,
    ...
    <Drawer open={isOpen} onOpenChange={onClose} direction="right">
      <DrawerContent className="h-full w-[850px] ml-auto">
          ...
          {mostrarExito && (
            <Alert className="border-green-200 bg-green-50 sticky top-0 z-50 shadow-md">
              ...
              <AlertDescription className="space-y-1">
                <p className="font-medium text-green-800">¡Salida guardada exitosamente!</p>
          ...
          <section className="space-y-4">
              ...
              <div>
                ...
                <div className="mt-1">
                  <CustomCombobox
                    ...
                    emptyMessage="Sin insumos con stock."
            ...
            <div>
              <Label className="text-sm font-medium text-gray-700">
                Cantidad *
                {insumoSeleccionadoData && (
                  <span className="ml-2 text-xs text-gray-500">(Disponible: {cantidadDisponible})</span>
                )}
              </Label>
        ...
        <div className="border-t p-6 flex justify-end gap-3">
          ...
          <Button className="bg-red-600 hover:bg-red-700" disabled={loading} onClick={guardar}>
