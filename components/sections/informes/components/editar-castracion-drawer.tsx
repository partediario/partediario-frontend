...
export default function EditarCastracionDrawer({ isOpen, onClose, onSuccess, parte }: EditarCastracionDrawerProps) {
  ...
  const eliminarActividad = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.error || "Error al eliminar castración")
      }

      toast({
        title: "✅ Castración Eliminada",
        description: "La castración ha sido eliminada correctamente",
        duration: 4000,
      })
      ...
      console.error("Error deleting castración:", error)
      toast({
        title: "❌ Error",
        description: error instanceof Error ? error.message : "Error al eliminar castración",
        variant: "destructive",
      })
    } finally {
      setDeleting(false)
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
              ...
              <p className="text-gray-600 mb-6">
                ¿Está seguro que desea eliminar esta castración? Esta acción no se puede deshacer.
              </p>
