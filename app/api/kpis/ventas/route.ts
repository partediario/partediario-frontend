...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${supabaseUrl}/rest/v1/pd_movimientos_animales_agg1_view?establecimiento_id=eq.${establecimientoId}&tipo_movimiento=eq.SALIDA&movimiento=eq.Venta&select=cantidad_animales`
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: "Error consultando ventas" }, { status: response.status })
    }
